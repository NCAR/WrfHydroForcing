...
def jday_for_date(in_date):
    # determine day-of-year
    jday = in_date.timetuple().tm_yday

    # adjust as necessary for leap year
    adjusted = False
    leap_year = datetime(in_date.year, 12, 31).timetuple().tm_yday == 366
...
def apply_daymet_bias_correction(input_forcings, config_options, mpi_config, force_num, geo_meta, description, variable, operator):
    ...
    if mpi_config.rank == 0:
        jday, adjusted = jday_for_date(input_forcings.fcst_date1)

        if mpi_config.rank == 0:
            config_options.statusMsg = f"Performing CONUS404/Daymet {description} bias corrrection " \
                                       f"for parameter day {jday} {f'(actual day {jday-1}) ' if adjusted else ''}" \
                                       f"of year {input_forcings.fcst_date1.year}"
        ...
        try:
            ...
            if mpi_config.rank == 0:
                config_options.statusMsg = f"Loading bias corrections from file: {offset_file_path}"
                err_handler.log_msg(config_options, mpi_config)
