...
errTitle = 'Error_get_MRMS_GC'
warningTitle = 'Warning_get_MRMS_GC'

pid = os.getpid()
lockFile = tmpDir + "/GET_MRMS_GC.lock"
...
if os.path.isfile(lockFile):
	...
	warningMsg =  "WARNING: Another MRMS GC Fetch Program Running. PID: " + pid
...
for hour in range(cleanBackHours, lookBackHours, -1):
	...
	fileClean = outDir + "/MRMS_GaugeCorr_QPE_01H_00.00_" + dCurrent.strftime('%Y%m%d') + \
...
for hour in range(lookBackHours,lagBackHours,-1):
	...
	fileDownload = "MRMS_GaugeCorr_QPE_01H_00.00_" + dCycle.strftime('%Y%m%d') + \
